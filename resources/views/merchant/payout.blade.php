...
<div class="container-fluid">
    <div class="header  ">
        <div class="dashboard-header">
            ...
            <button class="dashboard-button btn">Active balance @if (isset($balance))
                    ₹{{ sprintf('%.2f', $balance) }}
                @else
                    ₹0.00
                @endif </button>
                                        ...
                                        @isset($transactions)
                                            @foreach ($transactions as $trx)
                                                <tr>
                                                    ...
                                                    <td>₹{{ $trx->amount }}</td>
                                                    <td>₹{{ $trx->charge }}</td>
                                                    <td>{{ $trx->gst == 0 ? 'Not Applied': '₹'.$trx->gst }}</td>
                                                    <td>₹{{ $trx->gst_inc_charge == 0 ? $trx->charge : $trx->gst_inc_charge }}</td>
                                                    <td>
                                                        @php
...
<div id="initiateModal" class="modal fade" tabindex="-1" aria-labelledby="initiateModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <form class="modal-content" method="POST" action="{{ url('/merchant/initiate/payout') }}">
            @csrf
